...
export default function DashboardClient({ initialUser, initialLocale = "ar", initialView = "overview" }) {
  ...
  const fleet = agent.system?.fleet || { healthy: 0, total: processes.length, components: [] };
  const componentByName = new Map(
    (fleet.components || []).map((component) => [component.name, component])
  );
  const healthyComponents = Number.isFinite(Number(fleet.healthy))
    ? Number(fleet.healthy)
    : processes.filter((item) => item.status === "online").length;
  const totalComponents = Number(fleet.total || processes.length || 3);
  const networkCards = [
    {
  ...
  return <main className="admin-shell" dir={locale === "ar" ? "rtl" : "ltr"} lang={locale}>
    ...
    <div className="admin-workspace">
      ...
      <div className="admin-content">
        ...
        {view === "bots" && <section className="admin-section"><SectionTitle eyebrow={t.processControl} title={t.discordBots} note={t.processNote}/><div className="bot-grid">{processes.map((processItem) => { const label = BOT_LABELS[processItem.name]; const isOnline = processItem.status === "online"; const component = componentByName.get(processItem.name); const controllable = component?.controllable ?? processItem.name !== "andonis-games-bot"; const runtimeReady = component?.runtime?.ready ?? isOnline; return <article className={`bot-card ${label.accent}`} key={processItem.name}><div className="bot-head"><span className="bot-emblem"><Icon name={label.icon}/></span><div><h3>{label.title}</h3><p>{label.subtitle[locale]} · {controllable ? t.stableRelease : t.readOnly}</p></div><span className={`status-badge ${processItem.status}`}><i/>{processItem.status}</span></div><div className="bot-stats"><span><small>{t.uptime}</small><b>{formatUptime(processItem.uptimeStartedAt)}</b></span><span><small>{t.memory}</small><b>{formatBytes(processItem.memoryBytes)}</b></span><span><small>{t.cpu}</small><b>{Number(processItem.cpuPercent || 0).toFixed(1)}%</b></span><span><small>{t.restarts}</small><b>{processItem.restarts || 0}</b></span><span><small>{t.componentVersion}</small><b>{component?.version || processItem.version || "—"}</b></span><span><small>{t.readiness}</small><b className={runtimeReady ? "readiness-ready" : "readiness-degraded"}>{runtimeReady ? t.ready : t.degraded}</b></span></div><div className="bot-actions"><button className="start" disabled={!controllable || !agent.online || isOnline || Boolean(busy)} onClick={() => queueCommand({ type: "process.start", target: processItem.name })}>{busy === `process.start:${processItem.name}` ? t.queuing : t.start}</button><button disabled={!controllable || !agent.online || !isOnline || Boolean(busy)} onClick={() => setConfirmAction({ type: "process.restart", target: processItem.name, title: label.title })}>{t.restart}</button><button className="danger" disabled={!controllable || !agent.online || !isOnline || Boolean(busy)} onClick={() => setConfirmAction({ type: "process.stop", target: processItem.name, title: label.title })}>{t.stop}</button></div></article>; })}</div></section>}
